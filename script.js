/* ==========================================================================
   Nguyen Cong Dat — homepage behaviour
   1. theme toggle (system default, remembered override)
   2. ambient diffusion canvas, retuned to the warm palette and theme-aware
   3. scroll reveal
   4. active-section nav state
   ========================================================================== */
...
(() => {
  ...
  if (canvas && !reduceMotion.matches) {
    ...
    if (context) startCanvas(canvas, context);
  } else if (canvas) {
    canvas.hidden = true;
  }

  function startCanvas(element, context) {
    let width = 0;
    let height = 0;
    let frameId = 0;
    let particles = [];
    ...
    // Warm palettes drawn from the ElevenLabs cream/sand/blue family.
    const lightPalettes = [
      [[119, 113, 105], [143, 136, 127], [165, 159, 151]],
      [[176, 154, 118], [196, 176, 142], [214, 199, 170]],
      [[43, 127, 255], [92, 155, 255], [138, 183, 255]],
      [[92, 88, 82], [116, 111, 104], [140, 134, 126]]
    ];

    const darkPalettes = [
      [[165, 159, 151], [190, 184, 175], [214, 209, 201]],
      [[232, 220, 200], [214, 199, 170], [196, 176, 142]],
      [[122, 169, 255], [150, 188, 255], [180, 208, 255]],
      [[140, 134, 126], [164, 157, 148], [188, 181, 172]]
    ];
    ...
    const baseOpacity = () => (isDark() ? 0.085 : 0.075);
    ...
    const drawParticle = (particle, radius, opacity, rgb) => {
    ...
    const draw = (now) => {
      ...
      particles.forEach((particle) => {
        ...
        const rgb = activePalettes[particle.cluster][particle.tone];
        ...
        const opacity = idle + nearCluster * 0.16;
        const roundnessTarget = gathering ? 0 : 1;
        particle.roundness += (roundnessTarget - particle.roundness) * 0.055;
        drawParticle(particle, particle.radius + nearCluster * 0.55, opacity, rgb);

        if (gathering && distanceToCluster < 62) {
          context.beginPath();
          context.moveTo(particle.x, particle.y);
          context.lineTo(targetX, targetY);
          context.strokeStyle = `rgba(${rgb[0]}, ${rgb[1]}, ${rgb[2]}, ${(1 - distanceToCluster / 62) * 0.04})`;
